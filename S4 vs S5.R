...
data_s5 <- read.csv("S5.csv") %>% mutate(Scenario = "Scenario 5")

# Combine the two datasets
combined_data <- bind_rows(data_s4, data_s5)
...
# Define APA-compliant colors for Scenario 4 and Scenario 5
my_colors <- c("Scenario 4" = "#4F81BD",    # muted blue
               "Scenario 5" = "#999999")    # medium gray

# Plot Scenario 4 vs Scenario 5
ggplot(combined_data, aes(x = Stages, group = Scenario)) +
  ...
  labs(
    title = "Cumulative Time Across Each Stage: Scenario 4 vs Scenario 5",
    x = "Process Stage",
